...
Runs against the disassembled scripts in
smrpgpatchbuilder/src/disassembler_output/battle_animation.
"""
...
# Add src to path so disassembler_output becomes importable.
...
BANK_DIR = SRC_DIR / "disassembler_output" / "battle_animation"
...
def load_scripts_from_bank(bank_module, bank_name: str) -> list[tuple[str, str, list]]:
    ...
    bank = getattr(bank_module, 'bank', None) or bank_module
    if hasattr(bank, 'scripts') and bank.scripts:
...
def discover_bank_dirs() -> list[tuple[str, str]]:
    """Find bank subdirectories under BANK_DIR.

    Returns list of (dir_name, bank_label) — e.g., ("35", "0x35").
    """
    banks: list[tuple[str, str]] = []
    for entry in sorted(BANK_DIR.iterdir()):
        if not entry.is_dir():
            continue
        if not (entry / "export.py").exists():
            continue
        banks.append((entry.name, f"0x{entry.name.upper()}"))
    return banks


def import_bank_export(dir_name: str):
    """Import disassembler_output.battle_animation.<dir_name>.export via importlib.

    importlib.import_module tolerates module path segments that start with a
    digit (which a regular `import` statement cannot express).
    """
    return importlib.import_module(
        f"disassembler_output.battle_animation.{dir_name}.export"
    )
...
def main():
    ...
    bank_dirs = discover_bank_dirs()
    if not bank_dirs:
        print(f"No bank directories found under {BANK_DIR}", file=sys.stderr)
        sys.exit(1)

    for dir_name, bank_label in bank_dirs:
        print(f"Loading bank {bank_label} ({dir_name})...")
        module = import_bank_export(dir_name)
        all_scripts.extend(load_scripts_from_bank(module, bank_label))
